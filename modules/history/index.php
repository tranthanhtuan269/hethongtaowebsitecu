...
$ip = getRealIpAddr();
echo "<div class=\"page-history\">";
echo "<div class=\"title_home\" style=\"margin-bottom:8px;\"><h2>"._HISTORY."</h2></div>";
echo "<div class=\"page-history-content\">";
	$perpage = 20;
	$page = intval(isset($_GET['page']) ? $_GET['page'] : (isset($_POST['page']) ? $_POST['page']:1));
	$offset = ($page-1) * $perpage;
	$countf = $db->sql_fetchrow_simple($db->sql_query_simple("SELECT COUNT(*) FROM ".$prefix."_history WHERE ip='$ip'"));
	$total = ($countf[0]) ? $countf[0] : 1;
	$pageurl = "index.php?f=history";
	
	$result_history = $db->sql_query_simple("SELECT prd_id FROM ".$prefix."_history WHERE ip='$ip' ORDER BY time DESC LIMIT $offset, $perpage");
	if($db->sql_numrows($result_history) > 0) {	
	$i=1;
	...
	echo "</tr>";	
	while(list($prdid) = $db->sql_fetchrow_simple($result_history)) {	
	...
	if($db->sql_numrows($result_prd) > 0) {	
		list($idot, $titleot, $priceot) = $db->sql_fetchrow_simple($result_prd);
			...
			if($i%2==0){echo "<tr style=\"background:#FAFAFA;\">";}
				...
				echo"<td class=\"td-c\">";
					if($priceot == 0){echo "<div class=\"price\" style=\" color: #FF0000;
    font-size: 14px;
    font-weight: bold;\">"._DANG_CAP_NHAT." </div>";}
					else {echo "<div class=\"price\" style=\" color: #FF0000;
    font-size: 14px;
    font-weight: bold;\">".dsprice($priceot)." VND</div>";}
				echo "</td>";
				
				echo"<td class=\"td-d\" align=\"center\">";
					echo "<a href=\"index.php?f=history&do=delete&page=$page&pid=$idot\" info=\""._DELETE."\" onclick=\"return confirm('"._DELETEASK."');\"><img border=\"0\" src=\"images/delete.gif\"></a>";
				echo "</td>";
			echo "</tr>";	
	...
	//echo "</div>";
		...
		echo "<br><br><center><span style=\"color:#FF6600;\">"._NO_HISTORY."</span></center>";
	}
