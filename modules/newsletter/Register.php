...
$db->sql_query_simple("DELETE FROM {$prefix}_newsletter WHERE time + ".strval($newsletter_activation_interval * 3600)." <= UNIX_TIMESTAMP(NOW()) AND activateCode IS NOT NULL");
if ($db->sql_numrows($db->sql_query_simple("SELECT id FROM {$prefix}_newsletter WHERE email='$email'")) <= 0) {
	$db->sql_query_simple("INSERT INTO {$prefix}_newsletter (email, status, html, checkkey, time, activateCode) VALUES ('$email', 2, 1, '$rcode', '".TIMENOW."', '$rcode')");
	if ($db->sql_affectedrows() > 0) {
		$db->sql_query_simple("SELECT content FROM {$prefix}_gentext WHERE textname='sign' AND alanguage='$currentlang'");
		list($sign) = $db->sql_fetchrow_simple();
