...
$timecheck = time() - 600;
$db->sql_query_simple("DELETE FROM ".$prefix."_survey_check WHERE vottime<=$timecheck");

$attime = time();

$id = intval($_GET['id']);
$result = $db->sql_query_simple("SELECT question, votes, anwsers, options, totalvotes, time FROM ".$prefix."_survey WHERE active='1' AND id='$id' AND alanguage='$currentlang'");
if(empty($id) || $db->sql_numrows($result) != 1) {
	die();
}	

list($title, $votes, $anwsers, $options, $totalvotes, $time) = $db->sql_fetchrow_simple($result);
...
if($vote !="") {
	...
	$check = $db->sql_numrows($db->sql_query_simple("SELECT*FROM ".$prefix."_survey_check WHERE agent='$clientgs' AND vid='$id'"));
	if($check > 0) {
		...
		$db->sql_query_simple("UPDATE ".$prefix."_survey SET votes='$voteup', totalvotes=totalvotes+1 WHERE id='$id'");
		$db->sql_query_simple("INSERT INTO ".$prefix."_survey_check (vottime, vid, agent) VALUES ('".TIMENOW."', '$id', '$clientgs')");	
		header("Location: ".url_sid("index.php?f=$module_name&do=result&id=$id")."");
		exit;
	}
} else {
$db->sql_query_simple("UPDATE ".$prefix."_survey SET hits=hits+1 WHERE id='$id'");	
echo "<html>";
echo "<head>";
...
echo "<tr><td align=right style=\"font-family: 'Verdana'; font-size: 8pt; color: #ffffff; text-decoration: none\">".NameDay($attime).", ".ext_time($attime,2)." GMT+7</td></tr>";
