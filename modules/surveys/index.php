...
$result = $db->sql_query_simple("SELECT id, question, totalvotes, time FROM ".$prefix."_survey WHERE question!='' AND alanguage='$currentlang' ORDER BY time DESC");
if($db->sql_numrows($result) > 0) {
	echo "<table border=\"0\" width=\"100%\" cellpadding=\"3\" style=\"border-collapse: collapse\">";
	while(list($id, $title, $totalvotes, $time) = $db->sql_fetchrow_simple($result)) {
		echo "	<tr>";
		echo "		<td width=\"12\">";
