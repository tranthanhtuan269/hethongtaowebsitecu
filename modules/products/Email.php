...
$id = intval($_GET['id']);
$result = $db->sql_query_simple("SELECT images FROM {$prefix}_news WHERE id=$id AND alanguage='$currentlang'");
if(empty($id) || $db->sql_numrows($result) != 1) die();

require("templates/$Default_Temp/index.php");

$err_mail1 = $err_mail2 = "";
$sdemail = $rsemail = $sdname = $messenger = "";
$err = 0;

	$result_prd = $db->sql_query_simple("SELECT title, images FROM ".$prefix."_products WHERE id=$id");
	list($title, $images) = $db->sql_fetchrow_simple($result_prd);
...
if( isset($_POST['subup']) && $_POST['subup'] == 1) {
	...
	if(!$err) {
		...
		$mess .= "<link rel=\"StyleSheet\" href=\"$folder_site/css/".$Default_Temp.".css\" type=\"text/css\">";
		$mess .= "</head>";
		$mess .= "<table border=\"0\" width=\"100%\" cellpadding=\"5\" style=\"border-collapse: collapse\">";
		$mess .= "	<tr>";
		$mess .= "		<td>"._HELLO.",<br>";
		if($sdname !="") {
			$mess .=_SENDFR1."<a href=\"$folder_site/".fetch_urltitle("index.php?f=".$module_name."&do=detail&id=$id",$title)."\"><font color=\"red\">$title</font></a>"._FROM."<b>$sdname</b>";
		}else{
			$mess .= _SENDFR1."<a href=\"$folder_site/".fetch_urltitle("index.php?f=".$module_name."&do=detail&id=$id",$title)."\"><font color=\"red\">$title</font></a>"._FROM._SENDFR1_1;
		}
		if($messenger !="") {
			$mess .=_SENDFR2."<br>&quot;$messenger&quot;";
		}else{
			$mess .="";
		}
		$mess .="<br>"._SENDFR3."";
		$mess .="<br><a href=\"$folder_site/".fetch_urltitle("index.php?f=".$module_name."&do=detail&id=$id",$title)."\" title=\"".$title."\"><img src=\"$folder_site/files/".$module_name."/$images\" style=\"width:150px;\"></a>";
		$mess .="<br><a href=\"$folder_site/".fetch_urltitle("index.php?f=".$module_name."&do=detail&id=$id",$title)."\"><font color=\"red\">$title</font></a>";
		$mess .="<br>"._SENDFR4."";
		$mess .= "</td>";
		$mess .= "</tr>";
		$mess .= "</table>";
...
echo "	</tr>";
echo "	<tr>";
echo "		<td align=\"right\">"._MESSENGER.":</td>";
echo "		<td><textarea name=\"messenger\" cols=\"30\" rows=\"5\">$messenger</textarea></td>";
echo "	</tr>";
echo "	<tr>";
echo "		<td>&nbsp;</td>";
