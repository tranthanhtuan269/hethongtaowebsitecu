...
$userid = $userInfo['id'];
    ...
    echo "<form action=\"index.php?f=history&do=orders\" method=\"POST\" enctype=\"multipart/form-data\" \">";
	

echo "<tr>";
echo "<td width=\"120px\" align=\"right\" class=\"row1\"><b>Email: </b></td>\n";
echo "<td class=\"row2\">$err_email<input type=\"text\" id=\"email\" name=\"email\" value=\"$email\" size=\"100\"></td>\n";
echo "</tr>\n";
...
echo "<div class=\"div-home\">";
	...
	list ($xid) = $db->sql_fetchrow_simple($db->sql_query_simple("SELECT max(id) AS xid FROM ".$prefix."_products_order"));

	
	list ($mail) = $db->sql_fetchrow_simple($db->sql_query_simple("SELECT mail FROM ".$prefix."_products_order WHERE id=$xid "));
	

	
	$countf = $db->sql_fetchrow_simple($db->sql_query_simple("SELECT COUNT(*) FROM ".$prefix."_products_order WHERE mail='$mail'"));
	...
	$result = $db->sql_query_simple("SELECT title, fullname, mail, phone, address, info, orderList, DATE_FORMAT(orderTime,'%d/%c/%Y %T'), status FROM {$prefix}_products_order WHERE mail='$mail' ");
if($db->sql_numrows($result) > 0) {
	...
	$result_orders = $db->sql_query_simple("SELECT userid, sale, orderList, DATE_FORMAT(orderTime,'%d/%c/%Y %T'), status FROM {$prefix}_products_order WHERE mail='$mail' ORDER BY orderTime DESC LIMIT $offset, $perpage");
