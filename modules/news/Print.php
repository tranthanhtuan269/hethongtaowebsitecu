...
$id = intval($_GET['id']);
$result = $db->sql_query_simple("SELECT title, time, hometext, bodytext, images, imgtext, source FROM ".$prefix."_news WHERE id='$id'");
...
list($title, $time, $hometext, $bodytext, $images, $imgtext, $source) = $db->sql_fetchrow_simple($result);
$auts  ="";
if($source !="") {
	if($source !="") {
		$auts .= "(<i>$source</i>)";
...
<img border=\"0\" src=\"$path_upload_img/$images\" width=\"$sizenews\"></a></td>
	...
	<tr>
		<td class=\"loadbox\"><img border=\"0\" src=\"$urlsite/$path_upload_img/$images\"></td>
	</tr>$imgtext
</table>";
}
}else{
...
echo "<tr><td height=\"22\" align=\"center\">"._NEWSURL.": $siteurl/".url_sid("index.php?f=".$module_name."&do=detail&id=$id")."</td></tr>";
