...
echo "</td>";
$result_mntop = $db->sql_query_simple("SELECT title, url, target FROM ".$prefix."_mainmenus WHERE active='1' AND alanguage='$currentlang' ORDER BY weight");
$i  =0;
if($db->sql_numrows($result_mntop) > 0) {
	while(list($titlemntop, $urlmntop, $targetmntop) = $db->sql_fetchrow_simple($result_mntop)) {
		$i ++;
		if($targetmntop == 1) {
