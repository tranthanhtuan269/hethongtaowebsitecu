...
$result = $db->sql_query_simple("SELECT question, votes, alanguage, anwsers, options FROM ".$prefix."_survey WHERE id='$id'");

if(empty($id) || $db->sql_numrows($result) != 1) die();

list($title, $votes1, $alanguage, $anwsers1, $numas) = $db->sql_fetchrow_simple($result);	
...
if(isset($_POST['subup']) && $_POST['subup'] == 1) {
	...
	if(!$err) {	
		...
		$db->sql_query_simple("UPDATE ".$prefix."_survey SET question='$title', votes='$votes_up', totalvotes='$totalvotes', anwsers='$anwsers_up', options='".sizeof($anwsers_vl)."', alanguage='$alanguage' WHERE id='$id'");
...
if($stop) {
echo "<table align=\"center\" border=\"0\" width=\"100%\" cellspacing=\"0\" cellpadding=\"4\" class=\"tableborder table table-bordered\">\n";
