...
$total = $db->sql_numrows($db->sql_query_simple("SELECT*FROM ".$prefix."_survey WHERE alanguage='$currentlang'"));
$result = $db->sql_query_simple("SELECT  id, question, anwsers, time, active, hits  FROM ".$prefix."_survey WHERE alanguage='$currentlang' ORDER BY time DESC LIMIT $offset, $perpage");
...
echo "</tr>\n";
$i =0;
while(list($id, $question, $anwsers, $time, $active, $hits) = $db->sql_fetchrow_simple($result)) {
