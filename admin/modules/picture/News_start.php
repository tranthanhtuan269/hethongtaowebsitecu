...
if($stat == 0) {
	...
	if ($_GET['type'] == 'normal') $db->sql_query_simple("UPDATE {$prefix}_picture_cat SET startid=0 WHERE catid=$catid AND startid=$id AND alanguage='$currentlang'");
} else if ($stat == 1) {
	...
	if ($_GET['type'] == 'normal') $db->sql_query_simple("UPDATE {$prefix}_picture_cat SET startid=$id WHERE catid=$catid AND alanguage='$currentlang'");
}
