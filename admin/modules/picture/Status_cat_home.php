...
$result = $db->sql_query_simple("SELECT alanguage FROM ".$prefix."_picture_cat WHERE catid='$catid'");
if(empty($catid) || $db->sql_numrows($result) != 1) {
	header("Location: modules.php?f=".$adm_modname."");
	die();
}	

$db->sql_query_simple("UPDATE ".$prefix."_picture_cat SET onhome='$stat' WHERE catid='$catid'");
