...
if (isset($_POST['subup']) && $_POST['subup'] == 1) {
	...
	if (!$err) {
		$db->sql_query_simple("UPDATE ".$prefix."_picture_cat SET title='$title', active='$active', parent=$parent,images='$images' WHERE catid='$catid'");
		fixweight_cat();
...
$result = $db->sql_query_simple("SELECT title, active, parent,images FROM ".$prefix."_picture_cat WHERE catid=$catid AND alanguage='$currentlang'");
if($db->sql_numrows($result) != 1) {
	header("Location: ".$adm_modname.".php");
	die();
}
list($title, $active, $parent,$images) = $db->sql_fetchrow_simple($result);
...
echo "<form method=\"POST\" action=\"modules.php?f=picture&do=edit_cat&catid=$catid\" enctype=\"multipart/form-data\" onsubmit=\"return check(this);\"><table border=\"0\" width=\"100%\" cellspacing=\"0\" cellpadding=\"4\" class=\"tableborder table table-bordered\">\n";
echo "<tr><td colspan=\"2\" class=\"header\">Danh mục &raquo; Chỉnh sửa</td></tr>";
echo "<tr>\n";
...
//hien thi tat ca cac cap danh muc tin tuc
	$resultcat = $db->sql_query_simple("SELECT catid, title FROM ".$prefix."_picture_cat WHERE parent='0' AND catid!='$catid' AND alanguage='$currentlang' ORDER BY weight");
	if($db->sql_numrows($resultcat) > 0) {
		echo "<tr bgcolor=\"$scolor1\">\n";
		echo "<td width=\"40%\" align=\"right\" class=\"row1\"><b>Thuộc danh mục</b></td>\n";
		echo "<td class=\"row2\"><select name=\"parent\">";
		echo "<option name=\"catid\" value=\"0\">Danh mục cha</option>";
		$listcat ="";
