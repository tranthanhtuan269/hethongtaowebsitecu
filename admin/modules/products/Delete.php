...
$result = $db->sql_query_simple("SELECT images FROM ".$prefix."_products WHERE id='$id'");
...
else
{
	if($admin_ar[0]=="admin" || $admin_ar[0]=="tuongdd")
	{
		...
		$db->sql_query_simple("DELETE FROM ".$prefix."_products WHERE id='$id'");
        $db->sql_query_simple("DELETE FROM ".$prefix."_property_value WHERE product_id='$id'");
