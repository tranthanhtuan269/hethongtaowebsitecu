...
$countf = $db->sql_fetchrow_simple($db->sql_query_simple("SELECT COUNT(*) FROM ".$prefix."_products WHERE alanguage='$currentlang' AND Psale='1'"));
$total = ($countf[0]) ? $countf[0] : 1;
$result = $db->sql_query_simple("SELECT id, title, time, active, hits FROM ".$prefix."_products WHERE Psale='1' AND alanguage='$currentlang' $sortby LIMIT $offset,$perpage");
if($db->sql_numrows($result) > 0) {
	...
	echo "<tr>\n";
	echo "<td class=\"row1sd\" width=\"20\" align=\"center\">".sortBy("modules.php?f=$adm_modname&do=Psale",1)."</td>\n";
	...
	echo "<td class=\"row1sd\" align=\"center\" width=\"100\">"._TIMEUP." ".sortBy("modules.php?f=$adm_modname&do=Psale",3)."</td>\n";
	echo "<td class=\"row1sd\" align=\"center\" width=\"60\">"._STATUS."</td>\n";
	echo "<td class=\"row1sd\" align=\"center\" width=\"80\">"._VIEW." ".sortBy("modules.php?f=$adm_modname&do=Psale",5)."</td>\n";
	...
	if($total > $perpage) {
		...
		$pageurl = "modules.php?f=".$adm_modname."&do=Psale&sort=$sort";
