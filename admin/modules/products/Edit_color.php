...
$result = $db->sql_query_simple("SELECT title, active, parentid FROM ".$prefix."_color WHERE id='$catid' AND alanguage='$currentlang'");
...
if( isset($_POST['subup']) && $_POST['subup'] == 1) {
	...
	if(!$err) {
		$db->sql_query_simple("UPDATE ".$prefix."_color SET title='$title', active='$active', parentid='$parentid' WHERE id='$catid'");
		fixweight_cat();
		fixsubcat();
		updateadmlog($admin_ar[0], $adm_modname, _MODTITLE, _EDITCAT);
		header("Location: modules.php?f=".$adm_modname."&do=color");
	}
}
