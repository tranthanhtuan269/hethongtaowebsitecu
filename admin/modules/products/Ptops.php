...
$countf = $db->sql_fetchrow_simple($db->sql_query_simple("SELECT COUNT(*) FROM ".$prefix."_products WHERE alanguage='$currentlang' AND ptops='1'"));
$total = ($countf[0]) ? $countf[0] : 1;
$result = $db->sql_query_simple("SELECT id, title, time, active, hits FROM ".$prefix."_products WHERE ptops='1' AND alanguage='$currentlang' $sortby LIMIT $offset,$perpage");
