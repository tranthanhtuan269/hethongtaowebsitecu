...
if( isset($_POST['subup'])&& $_POST['subup'] == 1) {
	...
	if(!$err) {
		$weight = WeightMax("color");
		$db->sql_query_simple("INSERT INTO ".$prefix."_color (id, parentid, title, alanguage, active, weight) VALUES (NULL, '0', '$title', '$currentlang', '$active', '$weight')");
...
$resultcat = $db->sql_query_simple("SELECT id, parentid, title, active, weight  FROM ".$prefix."_color WHERE parentid='0' AND alanguage='$currentlang' ORDER BY weight,id ASC");
if($db->sql_numrows($resultcat) > 0) {
	...
	while(list($catid, $parentid, $title, $active, $weight) = $db->sql_fetchrow_simple($resultcat)) {
		if($ajax_active != 1) {
			switch($active) {
				case 1: $active = "<a href=\"modules.php?f=$adm_modname&do=status_color&catid=$catid&stat=0\" title=\""._DEACTIVATE."\" onclick=\"return aj_base_status($catid,0,'$adm_modname','status_cat','catid');\"><img border=\"0\" src=\"images/view.png\"></a>"; break;
				case 0: $active = "<a href=\"modules.php?f=$adm_modname&do=status_color&catid=$catid&stat=1\" title=\""._ACTIVE."\" onclick=\"return aj_base_status($catid,1,'$adm_modname','status_cat','catid');\"><img border=\"0\" src=\"images/viewo.png\"></a>"; break;
			}
		} else {
			switch($active) {
				case 1: $active = "<a href=\"modules.php?f=$adm_modname&do=status_color&catid=$catid&stat=0\" info=\""._DEACTIVATE."\"><img border=\"0\" src=\"images/view.png\"></a>"; break;
				case 0: $active = "<a href=\"modules.php?f=$adm_modname&do=status_color&catid=$catid&stat=1\" info=\""._ACTIVE."\"><img border=\"0\" src=\"images/viewo.png\"></a>"; break;
			}
		}
		...
		if($ajax_active == 1) {
			$listcat .= "		<td class=\"row1\" id=\"".$adm_modname."_title_edit_".$catid."\"><a href=\"?f=$adm_modname&do=edit_color&catid=$catid\" title=\""._QUICK_EDIT."\" onclick=\"return show_edit_title($catid,'$title','$adm_modname',30,'"._SAVECHANGES."','quick_title_color');\"><b>$title</b></a> </td>\n";
		} else {
		...
		// $count_prd = $db->sql_query_simple("SELECT count(*) FROM {$prefix}_products_color WHERE  colorid=$catid");
		...
		$listcat .= "<td align=\"center\" class=\"row1\"><a href=\"modules.php?f=$adm_modname&do=edit_color&catid=$catid\" info=\""._EDIT."\"><img border=\"0\" src=\"../images/edit.gif\"></a></td>\n";
		if($ajax_active != 1) {
			$listcat .= "<td align=\"center\" class=\"row1\"><a href=\"modules.php?f=$adm_modname&do=delete_color&catid=$catid\" title=\""._DELETE."\" onclick=\"return aj_base_delete($catid,'$adm_modname','"._DELETEASK_COLOR."','delete_cat','catid');\"><img border=\"0\" src=\"images/delete.png\"></td>\n";
		} else {
			$listcat .= "<td align=\"center\" class=\"row1\"><a href=\"modules.php?f=$adm_modname&do=delete_color&catid=$catid\" info=\""._DELETE."\" onclick=\"return confirm('"._DELETEASK_COLOR."');\"><img border=\"0\" src=\"images/delete.png\"></td>\n";
		}
		
	}
	echo $listcat;
	echo "<input type=\"hidden\" name=\"do\" value=\"quick_do_color\">";
