...
if( isset($_POST['subup'])&& $_POST['subup'] == 1) {
	...
	if(!$err) {
		...
		// }
		$weight = WeightMax("company");
		$db->sql_query_simple("INSERT INTO ".$prefix."_company (id, title, alanguage, active, weight) VALUES (NULL, '$title', '$currentlang', '$active', '$weight')");
...
echo "<tr><td colspan=\"2\" class=\"header\">"._CREATEXUATXU."</td></tr>";
echo "<tr>\n";
...
// echo "</tr>\n";
// $result_cat = $db->sql_query_simple("SELECT id, title FROM ".$prefix."_company WHERE active='1' AND parentid='0' AND alanguage='$currentlang' ORDER BY weight");
// if($db->sql_numrows($result_cat) > 0) {
// 	echo "<tr>\n";
// 	echo "<td width=\"40%\" align=\"right\" class=\"row1\"><b>Thuộc phong cách</b></td>\n";
// 	echo "<td class=\"row2\"><select name=\"parentid\">";
// 	echo "<option name=\"catid\" value=\"0\">Chọn phong cách</option>";
// 	$listcat ="";
...
$resultcat = $db->sql_query_simple("SELECT id, parentid, title, active, weight, prdid  FROM ".$prefix."_company WHERE parentid='0' AND alanguage='$currentlang' ORDER BY weight,id ASC");
if($db->sql_numrows($resultcat) > 0) {
	...
	echo "<tr><td colspan=\"10\" class=\"header\">"._CURRENTXUATXU."</td></tr>";
	$listcat ="";
	$listcat .= "	<tr>\n";
	...
	// $listcat .= "<td align=\"center\" width=\"160\" class=\"row1sd\">Sản phẩm</td>\n";
	...
	$listcat .= "	</tr>\n";
	while(list($catid, $parentid, $title, $active, $weight, $prdid) = $db->sql_fetchrow_simple($resultcat)) {
		if($ajax_active != 1) {
			switch($active) {
				case 1: $active = "<a href=\"modules.php?f=$adm_modname&do=status_company&catid=$catid&stat=0\" title=\""._DEACTIVATE."\" onclick=\"return aj_base_status($catid,0,'$adm_modname','status_cat','catid');\"><img border=\"0\" src=\"images/view.png\"></a>"; break;
				case 0: $active = "<a href=\"modules.php?f=$adm_modname&do=status_company&catid=$catid&stat=1\" title=\""._ACTIVE."\" onclick=\"return aj_base_status($catid,1,'$adm_modname','status_cat','catid');\"><img border=\"0\" src=\"images/viewo.png\"></a>"; break;
			}
		} else {
			switch($active) {
				case 1: $active = "<a href=\"modules.php?f=$adm_modname&do=status_company&catid=$catid&stat=0\" info=\""._DEACTIVATE."\"><img border=\"0\" src=\"images/view.png\"></a>"; break;
				case 0: $active = "<a href=\"modules.php?f=$adm_modname&do=status_company&catid=$catid&stat=1\" info=\""._ACTIVE."\"><img border=\"0\" src=\"images/viewo.png\"></a>"; break;
			}
		}
		...
		if($ajax_active == 1) {
			$listcat .= "		<td class=\"row1\" id=\"".$adm_modname."_title_edit_".$catid."\"><a href=\"?f=$adm_modname&do=edit_company&catid=$catid\" title=\""._QUICK_EDIT."\" onclick=\"return show_edit_title($catid,'$title','$adm_modname',30,'"._SAVECHANGES."','quick_title_company');\"><b>$title</b></a> </td>\n";
		} else {
			$listcat .= "		<td class=\"row1\"><b>$title</b> </td>\n";
		}
		// $count_prd = $db->sql_query_simple("SELECT title FROM {$prefix}_products WHERE  id=$prdid");
		...
		$listcat .= "<td align=\"center\" class=\"row1\"><a href=\"modules.php?f=$adm_modname&do=edit_company&catid=$catid\" info=\""._EDIT."\"><img border=\"0\" src=\"../images/edit.gif\"></a></td>\n";
		if($ajax_active != 1) {
			$listcat .= "<td align=\"center\" class=\"row1\"><a href=\"modules.php?f=$adm_modname&do=delete_company&catid=$catid\" title=\""._DELETE."\" onclick=\"return aj_base_delete($catid,'$adm_modname','"._DELETEASK_XUATXU."','delete_cat','catid');\"><img border=\"0\" src=\"images/delete.png\"></td>\n";
		} else {
			$listcat .= "<td align=\"center\" class=\"row1\"><a href=\"modules.php?f=$adm_modname&do=delete_company&catid=$catid\" info=\""._DELETE."\" onclick=\"return confirm('"._DELETEASK_XUATXU."');\"><img border=\"0\" src=\"images/delete.png\"></td>\n";
		}

	}
	echo $listcat;
	echo "<input type=\"hidden\" name=\"do\" value=\"quick_do_company\">";
