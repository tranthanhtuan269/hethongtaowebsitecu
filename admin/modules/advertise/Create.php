...
if(isset($_POST['subup']) && $_POST['subup'] == 1) {
	...
	if(!isset($err)) {
		...
		$db->sql_query_simple("INSERT INTO ".$prefix."_advertise (id, bnid, title, links, time, target, images, imgtext, alanguage, weight, module) VALUES (NULL, '$bnid', '$title', '$links', '".time()."', '$target', '$images', '$imgtext', '$currentlang', '$weight', '$bmodule_up')");
		fixcountbn();
		fixweight();
...
if($db->sql_numrows($result_bn) > 0) {
	while(list($bnid, $titlebn) = $db->sql_fetchrow_simple($result_bn)) {
