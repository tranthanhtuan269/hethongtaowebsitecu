...
$bnid = intval($_GET['id']);
$result_bn = $db->sql_query_simple("SELECT title FROM ".$prefix."_advertise_banners WHERE bnid='$bnid'");
...
<?php
list($titlebn) = $db->sql_fetchrow_simple($result_bn);
...
$total = $db->sql_numrows($db->sql_query_simple("SELECT*FROM ".$prefix."_advertise WHERE bnid='$bnid' AND alanguage='$currentlang'"));
$result = $db->sql_query_simple("SELECT  id, title, images,time, active, hits, weight  FROM ".$prefix."_advertise WHERE bnid='$bnid' AND alanguage='$currentlang' ORDER BY weight LIMIT $offset, $perpage");
if($db->sql_numrows($result) > 0) {
	...
	echo "	}	\n";
	echo "</script>\n";
	echo "<form action=\"modules.php?f=$adm_modname&do=$do&id=$bnid&page=$page\" name=\"frm\" method=\"POST\" onsubmit=\"return checkQuick(this);\"><tr><td class=\"row3 hidden-xs\" width=\"10\"><input type=\"checkbox\" name=\"checkall\" onclick=\"javascript:check_uncheck();\" title=\""._CHECKALL."\"></td>\n";
	echo "<td class=\"row1sd\">"._TITLE."</td>\n";
	echo "<td class=\"row1sd\">Hình ảnh</td>\n";
	echo "<td align=\"center\" width=\"50\" class=\"row3sd\">"._WEIGHT."</td>\n";
	echo "<td class=\"row1sd hidden-xs\" align=\"center\" width=\"100\">"._DATESENT."</td>\n";
	echo "<td class=\"row3sd hidden-xs\" align=\"center\" width=\"80\">Clicks</td>\n";
	...
	$i =0;
	while(list($id, $title,$images, $time, $active, $hits, $weight) = $db->sql_fetchrow_simple($result)) {
		$src = "../$path_upload/adv/$images";
		switch($active) {
		...
		echo "<tr>\n";
		echo "<td class=\"row1 hidden-xs\"><input type=\"checkbox\" name=\"id[]\" value=\"$id\"></td>";
		echo "<td class=\"row3\"><a href=\"?f=$adm_modname&do=edit&id=$id\" info=\""._EDIT."\"><b>$title</b></a></td>\n";
		echo "<td class=\"row3\"> <b ><img class='imagese' src=".$src." /> </b> </td>\n";
		...
		echo "<td class=\"row3 hidden-xs\" align=\"center\">".ext_time($time, 2)."</td>\n";
		echo "<td class=\"row1 hidden-xs\" align=\"center\">$hits</td>\n";
