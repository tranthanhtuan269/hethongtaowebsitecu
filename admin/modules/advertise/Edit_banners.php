...
<?php
$id = intval($_GET['id']);
$result = $db->sql_query_simple("SELECT title, bwidth, bheight, abs, type FROM ".$prefix."_advertise_banners WHERE bnid='$id'");
if(empty($id) || $db->sql_numrows($result) != 1) {
	header("Location: modules.php?f=".$adm_modname."&do=banners");
	exit;
}

list($title, $bwidth, $bheight, $absbn, $bntype) = $db->sql_fetchrow_simple($result);
...
if(isset($_POST['subup']) && $_POST['subup'] == 1) {
	...
	if($db->sql_numrows($db->sql_query_simple("SELECT*FROM ".$prefix."_advertise_banners WHERE title='$title' AND bnid!='$id'")) > 0) {
	...
	if(!$err) {
		$db->sql_query_simple("UPDATE ".$prefix."_advertise_banners SET title='$title', bwidth='$bwidth', bheight='$bheight', abs='$absbn', type='$bntype' WHERE bnid='$id'");
		header("Location: modules.php?f=".$adm_modname."&do=banners");
