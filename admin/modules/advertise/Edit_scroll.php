...
$id = intval($_GET['id']);
$result = $db->sql_query_simple("SELECT title, links, images, imgtext, target, poz FROM ".$prefix."_advertise_scroll WHERE id='$id' AND alanguage='$currentlang'");
if(empty($id) || $db->sql_numrows($result) != 1) {
	header("Location: ".$adm_modname.".php?do=scroll");
	exit;
}

list($title, $links, $images, $imgtext, $target, $poz) = $db->sql_fetchrow_simple($result);
...
if(isset($_POST['subup']) && $_POST['subup'] == 1) {
	...
	if(!$err) {
		...
		$db->sql_query_simple("UPDATE ".$prefix."_advertise_scroll SET title='$title', links='$links', images='$img_up', imgtext='$imgtext', target='$target', poz='$poz' WHERE id='$id'");
		fixweight_scroll();
