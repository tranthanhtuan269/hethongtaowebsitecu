...
if (defined('iS_RADMIN')) {
	...
	if(isset($_POST['subup']) && $_POST['subup'] == 1) {
		...
		if($db->sql_numrows($db->sql_query_simple("SELECT*FROM ".$prefix."_advertise_banners WHERE title='$title'")) > 0) {
		...
		if(!$err) {
			$db->sql_query_simple("INSERT INTO ".$prefix."_advertise_banners (bnid, title, alanguage, bwidth, bheight, abs, type) VALUES (NULL, '$title', '$currentlang', '$bwidth', '$bheight', '$absbn', '$bntype')");
	...
	$result = $db->sql_query_simple("SELECT bnid, title, counts, active, bwidth, bheight, abs FROM ".$prefix."_advertise_banners ORDER BY bnid DESC");
	if($db->sql_numrows($result) > 0) {
		...
		echo "<tr>";
		echo "<td class=\"row1sd  hidden-xs\" width=\"10\"><input type=\"checkbox\" name=\"checkall\" onclick=\"javascript:check_uncheck();\" title=\""._CHECKALL."\"></td>\n";
		echo "<td class=\"row3sd\">"._TITLE."</td>\n";
		echo "<td class=\"row1sd hidden-xs\" align=\"center\" width=\"100\">"._METHOD."</td>\n";
		echo "<td class=\"row1sd hidden-xs\" align=\"center\" width=\"80\">"._BNSIZES."</td>\n";
		...
		echo "</tr>\n";
		while(list($bnid, $title, $counts, $active, $bwidth, $bheight, $absbn) = $db->sql_fetchrow_simple($result)) {
			if($absbn == 1) {
				$absck = "*";
			} else {
				$absck ="";
			...
			echo "<tr>\n";
			echo "<td class=\"row1  hidden-xs\" width=\"10\"><input type=\"checkbox\" name=\"bnid[]\" value=\"$bnid\"></td>\n";
			echo "<td class=\"row3\">";
			if($counts > 0) {
			...
			echo "</td>\n";
			echo "<td class=\"row1 hidden-xs\" align=\"center\">advertising($bnid)</td>\n";
			echo "<td class=\"row1 hidden-xs\" align=\"center\">$bwidth x $bheight $absck</td>\n";
