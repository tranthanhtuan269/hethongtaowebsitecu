...
if(isset($_POST['subup']) && $_POST['subup'] == 1) {
	...
	if(!$err) {
		...
		$db->sql_query_simple("INSERT INTO ".$prefix."_advertise_scroll (id, poz, title, links, time, target, images, imgtext, alanguage, weight) VALUES (NULL, '$poz', '$title', '$links', '".time()."', '$target', '$images', '$imgtext', '$currentlang', '$weight')");
		fixweight_scroll();
...
echo "<div id=\"".$adm_modname."_main\"><form action=\"modules.php?f=$adm_modname&do=$do&page=$page\" method=\"POST\" enctype=\"multipart/form-data\" onsubmit=\"return check(this);\"><table border=\"0\" width=\"100%\" cellspacing=\"0\" cellpadding=\"3\" class=\"tableborder table table-bordered\">\n";
...
$numsf = $db->sql_fetchrow_simple($db->sql_query_simple("SELECT COUNT(*) FROM ".$prefix."_advertise_scroll WHERE alanguage='$currentlang'"));
$total = ($numsf[0]) ? $numsf[0] : 1;
$result = $db->sql_query_simple("SELECT  id, title, time, active, hits, weight, poz  FROM ".$prefix."_advertise_scroll WHERE alanguage='$currentlang' ORDER BY poz,weight LIMIT $offset, $perpage");
if($db->sql_numrows($result) > 0) {
	...
	$i =0;
	while(list($id, $title, $time, $active, $hits, $weight, $poz) = $db->sql_fetchrow_simple($result)) {
		if ($ajax_active == 1) {
			switch($active) {
