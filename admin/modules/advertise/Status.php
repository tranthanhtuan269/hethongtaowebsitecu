...
$result = $db->sql_query_simple("SELECT bnid FROM ".$prefix."_advertise WHERE id='$id'");
if(empty($id) || $db->sql_numrows($result) != 1) {
	header("Location: ".$adm_modname.".php");
	exit;	
}

list($bnid) = $db->sql_fetchrow_simple($result);
	
$db->sql_query_simple("UPDATE ".$prefix."_advertise SET active='$stat' WHERE id='$id'");
