...
$id = intval($_GET['id']);
$result = $db->sql_query_simple("SELECT title, links, images, imgtext, target, bnid, module FROM ".$prefix."_advertise WHERE id='$id' AND alanguage='$currentlang'");
if(empty($id) || $db->sql_numrows($result) != 1) {
	header("Location: modules.php?f=$adm_modname");
	exit;
}

list($title, $links, $images, $imgtext, $target, $bnid, $bmodule) = $db->sql_fetchrow_simple($result);
...
if(isset($_POST['subup']) && $_POST['subup'] == 1) {
	...
	if(!$err) {
		...
		$db->sql_query_simple("UPDATE ".$prefix."_advertise SET title='$title', links='$links', images='$img_up', imgtext='$imgtext', target='$target', bnid='$bnid', module='$bmodule_up' WHERE id='$id'");
		fixweight();
		fixcountbn();
...
echo "<form action=\"modules.php?f=$adm_modname&do=$do&id=$id\" method=\"POST\" enctype=\"multipart/form-data\" onsubmit=\"return check(this);\"><table border=\"1\" class=\"tableborder table table-bordered\" width=\"100%\" cellspacing=\"0\" cellpadding=\"3\" style=\"border-collapse: collapse\" bordercolor=\"#CCE2FE\">\n";
...
if($images !="" && file_exists("../$path_upload/adv/$images")) {
	...
	echo "		<td><input type=\"file\" name=\"userfile\" size=\"40\"><img class='showa' src=".$src." /></td>\n";
	echo "	</tr>\n";
...
echo "<td class=\"row2\">";
	editorbasic("imgtext",$imgtext,"",200);
echo "</td>\n";
echo "</tr>\n";
echo "<tr>\n";
...
$result_bn = $db->sql_query_simple("SELECT bnid, title FROM ".$prefix."_advertise_banners WHERE active='1' ORDER BY title");
if($db->sql_numrows($result_bn) > 0) {
	while(list($bn_id, $titlebn) = $db->sql_fetchrow_simple($result_bn)) {
		if($bn_id == $bnid) {
			$seld =" selected";
		} else {
			$seld ="";
