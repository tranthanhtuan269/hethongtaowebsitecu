...
$result = $db->sql_query_simple("SELECT title, url,images, parentid, target FROM ".$prefix."_mainmenus WHERE mid='$mid2'");
if(empty($mid2) || $db->sql_numrows($result) != 1) {
	header("Location: $adm_modname.php");
	exit;
}

list($title, $url,$images, $parentid, $newwindow) = $db->sql_fetchrow_simple($result);
...
if(isset($_POST['subup']) && $_POST['subup'] == 1) {
	...
	if(!$err) {
		...
		$db->sql_query_simple("UPDATE ".$prefix."_mainmenus SET parentid='$parentid', title='$title', url='$url',images='$images_up', target='$newwindow' WHERE mid='$mid2'");
		fixweight_mn();
...
echo "<form action=\"modules.php?f=$adm_modname&do=$do&mid=$mid2\" method=\"POST\" onsubmit=\"return check(this);\" enctype=\"multipart/form-data\"><table border=\"0\" width=\"100%\" cellspacing=\"0\" cellpadding=\"4\" class=\"tableborder table table-bordered\">\n";
...
echo "</tr>\n";
$result_cat = $db->sql_query_simple("SELECT mid, title FROM ".$prefix."_mainmenus WHERE mid!='$mid2' AND parentid='0' AND alanguage='$currentlang' ORDER BY weight");
