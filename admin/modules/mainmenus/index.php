...
?>
    <style>
        @media screen and (max-width: 400px){
            #mainmenus_main form:nth-child(1) .table-bordered td{
                width: 100%;
                display: block;
            }
            #mainmenus_main form:nth-child(1) .table-bordered td:nth-child(1){
                text-align: left;
            }
...
if(isset($_POST['subup']) && $_POST['subup'] == 1) {
	...
	if(!$err) {
		...
		$db->sql_query_simple("INSERT INTO ".$prefix."_mainmenus (mid, parentid, title, url,images, alanguage, weight, target) VALUES (NULL, '$parentid', '$title', '$url','$images', '$currentlang', '$weight', '$newwindow')");
		fixweight_mn();
...
echo "<div id=\"".$adm_modname."_main\"><form action=\"modules.php?f=$adm_modname&page=$page\" method=\"POST\" onsubmit=\"return check(this);\" enctype=\"multipart/form-data\"><table border=\"0\" width=\"100%\" cellspacing=\"0\" cellpadding=\"4\" class=\"tableborder table table-bordered\">\n";
...
echo "</tr>\n";
$result_cat = $db->sql_query_simple("SELECT mid, title FROM ".$prefix."_mainmenus WHERE parentid='0' AND alanguage='$currentlang' ORDER BY weight");
...
$total = $db->sql_numrows($db->sql_query_simple("SELECT * FROM ".$prefix."_mainmenus WHERE parentid='0' AND alanguage='$currentlang'"));
$resultcat = $db->sql_query_simple("SELECT mid, title, active, weight, url, target  FROM ".$prefix."_mainmenus WHERE parentid='0' AND alanguage='$currentlang' ORDER BY weight,mid ASC LIMIT $offset, $perpage");
...
function childcat($mid, $text="-") {
	...
	$treeTemp ="";
	$result = $db->sql_query_simple("SELECT mid, title, active, weight, url, target FROM ".$prefix."_mainmenus WHERE parentid='$mid' ORDER BY weight,mid ASC");
	if($db->sql_numrows($result) > 0 ) {
		...
		while(list($mid, $title, $active, $weight, $url, $target) = $db->sql_fetchrow_simple($result)) {
			...
			$treeTemp .= "<tr>\n";
			$treeTemp .= "<td class=\"row3\" align=\"center\"><input type=\"checkbox\" name=\"mid[]\" value=\"$mid\"></td>\n";
			if($ajax_active == 1) {
