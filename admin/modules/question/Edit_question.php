...
$query = "SELECT catid, title, time, hometext, bodytext, seo_title, seo_description, seo_keyword, seo_tag, fattach, othershow, images, imgtext, active, alanguage, source, imgshow, nstart, special, image_highlight";
if ($_GET['type'] == 'normal') {
	$table = "{$prefix}_camnang";
	$query .= ", time";
} else {
	$table = "{$prefix}_camnang_temp";
...
list($catid, $title, $time, $hometext, $bodytext, $title_seo, $description_seo, $keyword_seo, $tag_seo, $fattach, $othershow, $images, $imgtext, $active, $alang, $source, $imgshow, $startid, $special, $time, $highlight) = $db->sql_fetchrow_simple($result);
...
$path_upload_attach = "$path_upload/camnang/attachs";//path upload file attach
$err_title = $err_cat = "";
if (isset($_POST['submit'])) {
	...
	if(!$err) {
		...
		if ($timed) $newUploadPath = "$path_upload/camnang/".get_path($postTime);
		else $newUploadPath = $path_upload_img;
		if (is_uploaded_file($_FILES['userfile']['tmp_name'])) {
			$upload = new Upload("userfile", $newUploadPath, $maxsize_up);
			$images1 = $upload->send();
			//resizeImg($images1, $newUploadPath, $sizecamnang);
		...
		if ((($_GET['type'] == 'normal') && (!$timed)) || (($_GET['type'] == 'timed') && ($timed))) {
			$query = "UPDATE $table SET catid=$catid, title='$title', hometext='$hometext', bodytext='$bodytext', seo_title='$title_seo', seo_description='$description_seo', seo_keyword='$keyword_seo', seo_tag='$tag_seo', fattach='$fattach1', othershow='$othershow', images='$images1', imgtext='$imgtext', source='$source', imgshow=$imgshow, image_highlight=$highlight, nstart=$startid, special=$special";
			...
			$db->sql_query_simple("INSERT INTO {$prefix}_camnang_temp (catid, title, alanguage, hometext, bodytext, seo_title, seo_description, seo_keyword, seo_tag, fattach, othershow, images, imgtext, active, source, imgshow, image_highlight, hits, nstart, special, timed) VALUES ($catid, '$title', '$currentlang', '$hometext', '$bodytext', '$title_seo', '$description_seo', '$keyword_seo', '$tag_seo', '$fattach', '$othershow', '$images', '$imgtext', $active, '$source', $imgshow, $highlight, 0, $startid, $special, FROM_UNIXTIME($postTime))");
		} elseif (($_GET['type'] == 'timed') && (!$timed)) {
			$db->sql_query_simple("DELETE FROM $table WHERE id=$id");
			$db->sql_query_simple("INSERT INTO {$prefix}_camnang (catid, title, alanguage, time, hometext, bodytext, seo_title, seo_description, seo_keyword, seo_tag, fattach, othershow, images, imgtext, active, source, imgshow, image_highlight, hits, nstart, special) VALUES ($catid, '$title', '$currentlang', ".time().", '$hometext', '$bodytext', '$title_seo', '$description_seo', '$keyword_seo', '$tag_seo', '$fattach', '$othershow', '$images', '$imgtext', $active, '$source', $imgshow, $highlight, 0, $startid, $special)");
		}
		if (($db->sql_affectedrows() > 0) && (!$timed)) {
			...
			if($startid == 1) {
				...
				$db->sql_query_simple("UPDATE {$prefix}_camnang SET nstart=0 WHERE id!=$lastInsertId AND catid=$catid");
				$db->sql_query_simple("UPDATE {$prefix}_camnang_cat SET startid=$lastInsertId WHERE catid=$catid");
			}
		}
...
$result_cat = $db->sql_query_simple("SELECT catid, title FROM ".$prefix."_camnang_cat WHERE parent='0' and alanguage='$currentlang' ORDER BY weight");
...
echo "</tr>\n";
echo "<tr>\n";
echo "<td align=\"right\" class=\"row1\"><b>"._CAMNANGTART."</b></td>\n";
...
echo "</tr>\n";
echo "<tr>\n";
echo "<td width=\"20%\" align=\"right\" class=\"row1\"><b>"._TAG_SEO."</b></td>\n";
echo "<td class=\"row2\"><input type=\"text\" name=\"tag_seo\" value=\"$tag_seo\" size=\"70\"></td>\n";
echo "</tr>\n";
...
echo "<tr>\n";
echo "<td align=\"right\" class=\"row1\"><b>"._CAMNANG_ATTACH_URL."</b></td>\n";
$disabled="";
if (empty($fattach)) 
{
...
echo "</tr>\n";
echo "<tr>\n";
echo "<td width=\"20%\" align=\"right\" class=\"row1\"><b>"._CAMNANG_ATTACH_FILE."</b></td>\n";
...
echo "<td width=\"20%\" align=\"right\" class=\"row1\"><b>"._CAMNANG_IMAGE_HIGHLIGHT."</b></td>\n";
...
echo "<td width=\"20%\" align=\"right\" class=\"row1\"><b>"._CAMNANG_OTHER_SHOW."</b><br><i>"._CAMNANG_OTHER_SHOW_DETAIL."</i></td>\n";
...
echo "</tr>\n";
echo "<tr>\n";
echo "<td width=\"20%\" align=\"right\" class=\"row1\"><b>"._TITLE_SEO."</b></td>\n";
echo "<td class=\"row2\"><input type=\"text\" name=\"title_seo\" value=\"$title_seo\" size=\"70\"></td>\n";
echo "</tr>\n";
echo "<tr>\n";
echo "<td width=\"20%\" align=\"right\" class=\"row1\"><b>"._DESCRIPTION_SEO."</b></td>\n";
echo "<td class=\"row2\"><textarea cols=\"70\" rows=\"3\" name=\"description_seo\">$description_seo</textarea></td>\n";
echo "</tr>\n";
echo "<tr>\n";
echo "<td width=\"20%\" align=\"right\" class=\"row1\"><b>"._KEYWORD_SEO."</b></td>\n";
echo "<td class=\"row2\"><input type=\"text\" name=\"keyword_seo\" value=\"$keyword_seo\" size=\"70\"></td>\n";
echo "</tr>\n";
echo "<tr>\n";
echo "<td width=\"20%\" align=\"right\" class=\"row1\"><b>"._CAMNANG_TIMED."</b></td>\n";
echo "<td class=\"row2\">";
$timedChecked = '';
...
for ($i = 1; $i <= 12; $i++) {
	...
	echo '<option value="'.$i."\"$monSelected>"._CAMNANG_MONTH." $i</option>\n";
}
