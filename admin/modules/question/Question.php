...
$catArr = array();
$cats = $db->sql_query_simple("SELECT catid, title FROM {$prefix}_question_cat");
while (list($cid, $ctitle) = $db->sql_fetchrow_simple()) $catArr[$cid] = $ctitle;
$total = $db->sql_numrows($db->sql_query_simple("(SELECT id FROM {$prefix}_question WHERE alanguage='$currentlang')"));

$result = $db->sql_query_simple("SELECT id, catid, title, content, time, active, hits FROM {$prefix}_question  WHERE alanguage='$currentlang' $sortby LIMIT $offset, $perpage");
if($db->sql_numrows($result) > 0) {
	...
	echo "<td class=\"row1sd\" align=\"center\" width=\"100\">"._TITLE."</td>\n";
	echo "<td class=\"row1sd\"  align=\"left\" width=\"\">"._CONTENT."</td>\n";
	echo "<td class=\"row1sd\" align=\"center\" width=\"90\">"._CAMNANG_CATEGORY."  </td>\n";
	echo "<td class=\"row1sd hidden-xs\" align=\"center\" width=\"85\">"._TIMEUP."</td>\n";
	echo "<td class=\"row1sd\" align=\"center\" width=\"56\">"._STATUS."</td>\n";
	echo "<td class=\"row1sd hidden-xs\" align=\"center\" width=\"30\">"._VIEW."</td>\n";
	echo "<td class=\"row1sd\" align=\"center\" width=\"30\">"._DELETE."</td>\n";
	echo "</tr>\n";
	$i = 0;
	if($page > 1) { $a = $perpage * $page - $perpage + 1;}
	while(list($id, $catid, $title, $content, $time, $active, $hits) = $db->sql_fetchrow_simple($result)) {
		if (($i % 2) == 1) $css = "row1";
		else $css ="row3";
