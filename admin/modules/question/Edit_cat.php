...
if (isset($_POST['subup']) && $_POST['subup'] == 1) {
	...
	if (!$err) {
		$db->sql_query_simple("UPDATE ".$prefix."_question_cat SET title='$title', active='$active', onhome='$onhome', homelinks='$homelinks', parent=$parent WHERE catid='$catid'");
		fixweight_cat();
...
$result = $db->sql_query_simple("SELECT title, active, onhome, homelinks, parent FROM ".$prefix."_question_cat WHERE catid=$catid AND alanguage='$currentlang'");
...
echo "<form method=\"POST\" action=\"modules.php?f=question&do=edit_cat&id=$catid\" onsubmit=\"return check(this);\"><table border=\"0\" width=\"100%\" cellspacing=\"0\" cellpadding=\"4\" class=\"tableborder table table-bordered\">\n";
...
//hien thi tat ca cac cap danh muc tin tuc
	$resultcat = $db->sql_query_simple("SELECT catid, title FROM ".$prefix."_question_cat WHERE parent='0' AND catid!='$catid' AND alanguage='$currentlang' ORDER BY weight");
