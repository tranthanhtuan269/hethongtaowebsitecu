...
$result = $db->sql_query_simple("SELECT id, qid, name, content, time, active FROM {$prefix}_answer  WHERE alanguage='$currentlang' $sortby  LIMIT $offset, $perpage");
if($db->sql_numrows($result) > 0) {
	...
	while(list($id, $qid, $name, $content, $time, $active) = $db->sql_fetchrow_simple($result)) {
		if (($i % 2) == 1) $css = "row1";
		else $css ="row3";
		...
		$result_q = $db->sql_query_simple("SELECT title FROM {$prefix}_question  WHERE id=$qid");
		list($titleq) = $db->sql_fetchrow_simple($result_q);
