...
if(isset($_POST['subup'])&& $_POST['subup'] == 1) {
	...
	if(!$err) {
		...
		$db->sql_query_simple("INSERT INTO ".$prefix."_question_cat (catid, title, alanguage, active, weight, onhome, homelinks, parent, counts, startid) VALUES (NULL, '$title',  '$currentlang', '$active', '$weight', '$onhome', '$homelinks', $parent, 0, 0)");
		fixweight_cat();
...
echo "<div id=\"{$adm_modname}_main\"><form action=\"modules.php?f=$adm_modname&do=$do\" method=\"POST\" onsubmit=\"return check(this);\"><table border=\"0\" width=\"100%\" cellspacing=\"0\" cellpadding=\"4\" class=\"tableborder table table-bordered\">\n";
...
$resultcat1 = $db->sql_query_simple("SELECT catid, title FROM {$prefix}_question_cat WHERE parent=0 AND alanguage='$currentlang' ORDER BY weight");
if($db->sql_numrows($resultcat1) > 0) 
{
	...
	$listcat ="";
	while(list($cat_id, $titlecat) = $db->sql_fetchrow_simple($resultcat1)) 
	{
		if ($cat_id == $parent) 
		{
			$seld =" selected"; 
...
function listCat($catArr, $newArr, $tdClass, $pad = '') {
	...
	foreach ($newArr as $key => $val) {
		...
		if ($ajax_active == 1) {
			...
			$title = "<a href=\"?f=$adm_modname&do=quick_title_cat&id={$tempArr['id']}\" info=\""._QUICK_EDIT."\" onclick=\"return show_edit_title({$tempArr['id']},'{$tempArr['title']}','$adm_modname',20,'"._SAVECHANGES."','quick_title_cat');\">{$tempArr['title']}</a> <a href=\"../".url_sid("index.php?f=".$adm_modname."&do=categories&id={$tempArr['id']}&t=".cv2urltitle($tempArr['title'])."")."\" info=\""._GETLINK."\" onclick=\"prompt('"._GETLINK."','".url_sid("index.php?f=$adm_modname&do=categories&id={$tempArr['id']}&t=".cv2urltitle($tempArr['title'])."")."'); return false;\"><img border=\"0\" src=\"images/link.png\"></a>";
...
$resultcat = $db->sql_query_simple("SELECT catid, title, active, weight, counts, startid, onhome, homelinks, parent FROM {$prefix}_question_cat WHERE alanguage='$currentlang' ORDER BY weight, catid ASC ");
