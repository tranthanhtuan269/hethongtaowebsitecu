...
$active = 1;
$title = $catid = $hometext = $bodytext = $iframe = $imgtext = $source = $tag_seo = $title_seo = $keyword_seo = $description_seo = $startid = $special = $imgshow = $err_cat = $err_title = $images = '';
if( isset($_POST['subup']) && $_POST['subup'] == 1) {
	...
	if (!$err) {
		...
		list ($xid) = $db->sql_fetchrow_simple($db->sql_query_simple("SELECT max(id) AS xid FROM ".$prefix."_news"));
		if ($xid == "-1") { $id = 1; } else { $id = $xid + 1; }
		if ($timed) $insertIntoTable = "{$prefix}_news_temp";
		else $insertIntoTable = "{$prefix}_news";
		$query = "INSERT INTO $insertIntoTable (id, catid, title, permalink, alanguage, hometext, bodytext, fattach, othershow, images, imgtext, tag_seo, title_seo, keyword_seo, description_seo, active, source, imgshow, image_highlight, hits, nstart, special";
		if ($timed) $query .= ', timed';
		else $query .= ', time';
		$query .= ") VALUES ($id, $catid, '$title',  '$permalink' ,'$currentlang', '$hometext', '$bodytext', '$fattach', '$othershow', '$images', '$imgtext', '$tag_seo', '$title_seo', '$keyword_seo', '$description_seo', $active, '$source', $imgshow, $highlight, 0, $startid, $special";
		...
		if (($db->sql_affectedrows() > 0) && (!$timed)) {
			fixcount_cat();
			if($startid == 1) {
				$db->sql_query_simple("UPDATE {$prefix}_news SET nstart=0 WHERE id!=$id AND catid=$catid");
				$db->sql_query_simple("UPDATE ".$prefix."_news_cat SET startid=$id WHERE catid=$catid");
			}
		}
...
echo "<td class=\"row2\"><input type=\"checkbox\" name=\"highlight\" value=\"1\" checked></td>\n";
echo "</tr>\n";
echo "<tr>\n";
