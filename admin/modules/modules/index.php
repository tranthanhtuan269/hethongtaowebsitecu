...
$result = $db->sql_query_simple("SELECT mid, title, custom_title, active, view  FROM ".$prefix."_modules WHERE alanguage='$currentlang' ORDER BY title");
if($db->sql_numrows($result) > 0) {
	ajaxload_content();

	
	echo "<div id=\"".$adm_modname."_main\"><table border=\"0\" width=\"100%\" align=\"center\"><tr><td><table border=\"0\" width=\"100%\" cellspacing=\"0\" cellpadding=\"5\" class=\"tableborder table table-bordered\">\n";
	...
	echo "</tr>\n";
	$i =0;
	while(list($mid, $title, $custom_title, $active, $view) = $db->sql_fetchrow_simple($result)) {
		if($ajax_active == 1) {
			switch($active) {
