...
$result = $db->sql_query_simple("SELECT moncard FROM {$prefix}_user  WHERE id=$id");
if($db->sql_numrows($result) >0) 
{
	list($mmoncard) = $db->sql_fetchrow_simple($result);
	$xien = $moncard+$mmoncard;
	$db->sql_query_simple("UPDATE {$prefix}_user SET moncard='$xien' WHERE id=$id");
