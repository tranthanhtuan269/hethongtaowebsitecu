...
$result = $db->sql_query_simple("SELECT id, userid, fullname, mail, orderList, DATE_FORMAT(orderTime,'%d/%c/%Y %T'), status FROM {$prefix}_products_order WHERE userid=".$userInfo['id']."  $sortby LIMIT $offset,$perpage");
if($db->sql_numrows($result) > 0) {
	...
	echo "<table border=\"0\" width=\"100%\" cellspacing=\"1\" style=\"background:#1F98FC\" cellpadding=\"4\" class=\"tableborder table table-bordered\">\n";
	...
	echo "</tr>\n";
	$i = 1;
	while(list($orderId, $userid, $fullname, $mail, $orderList, $orderTime, $status) = $db->sql_fetchrow_simple($result)) {
		if($i % 2 == 1) $css = "row3";
		else $css ="row1";
