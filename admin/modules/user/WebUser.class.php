...
class WebUser extends User {
	...
	function register($url) {
		...
		if (!empty($this->name) && !empty($this->email) && !empty($this->address) && !empty($this->phone)) {
			...
			if ($db->sql_numrows() > 0) {
				...
				$db->sql_query_simple("INSERT INTO {$this->tableName} (title, fullname, email, pass, address, phone, activationCode, registrationTime, loginAttempt) VALUES ({$this->sex}, '$name', '$email', '$pass', '$address', '$phone', '$activationCode', NOW(), 0)");
	...
	function getUser($id, $email = '') {
		...
		if (empty($email)) $db->sql_query_simple("SELECT id, title, fullname, email, pass, address, phone, loginAttempt FROM {$this->tableName} WHERE id=".intval($id));
		else $db->sql_query_simple("SELECT id, title, fullname, email, pass, address, phone, loginAttempt FROM {$this->tableName} WHERE email='".$escape_mysql_string($email)."'");

		if ($db->sql_numrows() > 0) {
			list($id, $title, $name, $email, $pass, $address, $phone, $loginAttempt) = $db->sql_fetchrow_simple();
