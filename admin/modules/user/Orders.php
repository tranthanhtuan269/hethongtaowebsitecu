...
$result = $db->sql_query_simple("SELECT userid, sale, orderList, DATE_FORMAT(orderTime,'%d/%c/%Y %T'), status FROM {$prefix}_products_order WHERE userid=$userid ORDER BY orderTime DESC");
if($db->sql_numrows($result) > 0) {
	while(list($userid, $sale, $orderList, $orderTime, $status) = $db->sql_fetchrow_simple($result)){
