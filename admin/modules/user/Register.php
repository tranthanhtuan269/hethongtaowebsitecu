...
//$db->sql_query_simple("DELETE FROM {$prefix}_user WHERE UNIX_TIMESTAMP(registrationTime) + ".strval($activationPeriod * 3600).' <= NOW() AND activationCode IS NOT NULL');
...
if (isset($_POST["submit_up"])) {
	...
	//if ($captcha->isValid($_POST['captcha'])) {
		$user = new WebUser($_POST['title'], $_POST['name'], $_POST['email'], $_POST['address'], $_POST['phone'], Hash::sha256($_POST['password']));
