...
?>
    <style>
        @media screen and (max-width: 400px){
            .main-page form:nth-child(2) .table-bordered td{
                width: 100%;
                display: block;
            }
            .main-page form:nth-child(2) .table-bordered td:nth-child(1){
                text-align: left;
            }
...
<?php
$id = intval($_GET['id']);
$result = $db->sql_query_simple("SELECT title, email,address,note,phone FROM ".$prefix."_contact_part WHERE id='$id' AND alanguage='$currentlang'");
if(empty($id) || $db->sql_numrows($result) != 1) {
	header("Location: modules.php?f=".$adm_modname.".&do=part");
}	

list($title, $email,$address,$note,$phone) = $db->sql_fetchrow_simple($result);
...
if(isset($_POST['subup']) && $_POST['subup'] == 1) {
	...
	$note = trim(stripslashes(resString($_POST['note'])));
	...
	if($db->sql_numrows($db->sql_query_simple("SELECT title FROM ".$prefix."_contact_part WHERE title='$title' AND id!='$id'")) > 0) {
		$ds_title = "";
		$err = 1;
	}
...
/*	if(!is_email($email)) {
	...
	if(!$err) {
		$db->sql_query_simple("UPDATE ".$prefix."_contact_part SET title='$title', email='$email',address='$address', note='$note', phone='$phone' WHERE id='$id'");
...
echo "<form action=\"modules.php?f=$adm_modname&do=$do&id=$id\" method=\"POST\" ><table border=\"0\" width=\"100%\" cellspacing=\"0\" cellpadding=\"3\" class=\"tableborder table table-bordered\">\n";
...
//echo "<td class=\"row3\">".errorMess($note, "note", '')."<input type=\"text\" name=\"note\" value=\"$note\" size=\"40\"></td>\n";
//echo "</tr>\n";

echo "<tr>\n";
echo "<td width=\"40%\" align=\"right\" class=\"row1\"><b>Số điện thoại</b></td>\n";
echo "<td class=\"row3\">".errorMess($phone, "phone", '')."<input type=\"text\" name=\"phone\" value=\"$phone\" size=\"40\"></td>\n";
echo "</tr>\n";
