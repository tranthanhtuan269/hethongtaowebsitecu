...
if (defined('iS_RADMIN')) {
	...
	$total = $db->sql_numrows($db->sql_query_simple("SELECT*FROM ".$prefix."_contact WHERE alanguage='$currentlang'"));
	$result = $db->sql_query_simple("SELECT  id,ctname,email,phone,content,ctdate,status   FROM ".$prefix."_contact WHERE alanguage='$currentlang' ORDER BY id DESC LIMIT $offset, $perpage");
	if($db->sql_numrows($result) > 0) {
		...
		echo "<td class=\"row1sd\" align=\"center\" width=\"150\">Nội dung</td>\n";
		echo "<td class=\"row3sd hidden-xs\" align=\"center\" width=\"100\">"._DATESENT."</td>\n";
		echo "<td class=\"row1sd hidden-xs\" align=\"center\" width=\"80\">"._STATUS."</td>\n";
		...
		$i =0;
		while(list($id, $ctname, $email, $phone, $content,$ctdate, $status) = $db->sql_fetchrow_simple($result)) {
			...
			echo "</td>\n";
			echo "<td align=\"center\" class=\"row1\"> $content</td>\n";
			echo "<td align=\"center\" class=\"row3 hidden-xs\"> $ctdate</td>\n";
			echo "<td align=\"center\" class=\"row1 hidden-xs\"><font color=\"red\">$status_o</font></td>\n";
