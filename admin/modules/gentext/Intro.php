...
$result = $db->sql_query_simple("SELECT content FROM ".$prefix."_gentext WHERE textname='intro' AND alanguage='$currentlang'");
list($content) = $db->sql_fetchrow_simple($result);

if($_POST['submit'] !="") {
	$content = trim(stripslashes(resString($_POST['content'])));
	if($db->sql_numrows($result) > 0) {
		$db->sql_query_simple("UPDATE ".$prefix."_gentext SET content='$content' WHERE textname='intro' AND alanguage='$currentlang'");
	}else{
		$db->sql_query_simple("INSERT INTO ".$prefix."_gentext (textname, content, alanguage) VALUES ('intro', '$content', '$currentlang')");
	}	
