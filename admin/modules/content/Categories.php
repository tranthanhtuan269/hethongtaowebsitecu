...
if(isset($_POST['subup'])&& $_POST['subup'] == 1) {
	...
	if(!$err) {
		...
		$db->sql_query_simple("INSERT INTO ".$prefix."_content_cat (catid, title, alanguage, active, weight, onhome, homelinks, parent, counts, startid) VALUES (NULL, '$title', '$currentlang', '$active', '$weight', '$onhome', '$homelinks', $parent, 0, 0)");
		fixweight_cat();
...
$resultcat = $db->sql_query_simple("SELECT catid, title FROM {$prefix}_content_cat WHERE parent=0 AND alanguage='$currentlang' ORDER BY weight");
...
function listCat($catArr, $newArr, $tdClass, $pad = '') {
	...
	foreach ($newArr as $key => $val) {
		...
		if ($ajax_active == 1) {
			...
			$title = "<a href=\"?f=$adm_modname&do=quick_title_cat&id={$tempArr['id']}\" info=\""._QUICK_EDIT."\" onclick=\"return show_edit_title({$tempArr['id']},'{$tempArr['title']}','$adm_modname',20,'"._SAVECHANGES."','quick_title_cat');\">{$tempArr['title']}</a>";
...
$resultcat = $db->sql_query_simple("SELECT catid, title, active, weight, counts, startid, onhome, homelinks, parent FROM {$prefix}_content_cat WHERE alanguage='$currentlang' ORDER BY weight, catid ASC");
if($db->sql_numrows($resultcat) > 0) {
