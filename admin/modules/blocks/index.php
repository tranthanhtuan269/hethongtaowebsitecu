...
if (defined('iS_ADMIN')) {
	...
	$result = $db->sql_query_simple("SELECT bid, bkey, title, url, bposition, weight, active, blockfile, view, link, module FROM ".$prefix."_blocks WHERE blanguage='$currentlang' ORDER BY bposition, weight");
	if($db->sql_numrows($result) > 0) {
		...
		echo "<div id=\"".$adm_modname."_main\"><form name=\"frm\" action=\"modules.php?f=".$adm_modname."\" method=\"POST\" onsubmit=\"return checkQuick(this);\"><table align=\"center\" border=\"0\" width=\"\" cellspacing=\"0\" cellpadding=\"3\" class=\"tableborder table table-bordered\">\n";
		...
		echo "</tr>\n";
		$i =0;
		while(list($bid, $bkey, $title, $url, $bposition, $weight, $active, $blockfile, $view, $link, $bmodule) = $db->sql_fetchrow_simple($result)) {
			switch($bposition) {
		...
		echo "</select> <input type=\"submit\" value=\""._DOACTION."\">&nbsp;<a href=\"modules.php?f=".$adm_modname."&do=add\"><input type=\"button\" value=\""._BLOCKSADD."\" onclick=\"window.location='modules.php?f=".$adm_modname."&do=add'\"></a></td></tr>";
		echo "</table></form></div>";
