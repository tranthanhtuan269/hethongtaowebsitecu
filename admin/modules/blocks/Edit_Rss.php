...
$result = $db->sql_query_simple("SELECT sitename, headlinesurl FROM ".$prefix."_rss WHERE rid='$id'");
...
list($sitename, $url) = $db->sql_fetchrow_simple($result);
$err_site = $err_url ="";
if(isset($_POST['submit']) && $_POST['submit'] !="") {
	...
	if($db->sql_numrows($db->sql_query_simple("SELECT rid FROM ".$prefix."_rss WHERE headlinesurl='$url' AND rid!='$id'")) > 0) {
		$err_url = "<font color=\"red\">"._ERROR5_1."</font><br>";
		$err = 1;
	}		
	
	if(!$err) {
		$db->sql_query_simple("UPDATE ".$prefix."_rss SET sitename='$sitename', headlinesurl='$url' WHERE rid='$id'");
