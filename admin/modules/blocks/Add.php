...
if(empty($tip)) {
	...
	if(isset($_POST['submit']) && !empty($_POST['submit'])) {
		...
		$result = $db->sql_query_simple("SELECT weight FROM ".$prefix."_blocks WHERE bposition='$bposition' AND blanguage='$currentlang' ORDER BY weight DESC");
		$row = $db->sql_fetchrow_simple($result);
		$weight = $row['weight'];
		$weight++;
		...
		if(!$err) {
			...
			$db->sql_query_simple("INSERT INTO ".$prefix."_blocks (bid, bkey, title, url, bposition, weight, active, refresh, time, blanguage, blockfile, view, expire, action, link, module, showtitle) VALUES (NULL, '$bkey', '$title', '$url', '$bposition', '$weight', '$active', '$refresh', '$btime', '$currentlang', '$blockfile', '$view', '$expire', '$action', '$link2', '$bmodule_up', '$showtitle')");
			fixweight();
			blist();
	...
	if($tip==3) {
		...
		$res = $db->sql_query_simple($sql);
		while($row = $db->sql_fetchrow_simple($res)) {
			$hid = $row['rid'];
			$htitle = $row['sitename'];
