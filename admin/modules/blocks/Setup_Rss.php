...
$result = $db->sql_query_simple("SELECT rid, sitename, headlinesurl FROM ".$prefix."_rss ORDER BY rid DESC");
...
echo "</tr>\n";
$i =0;
while(list($rid, $site_name, $url) = $db->sql_fetchrow_simple($result)) {
...
if(isset($_POST['submit']) && $_POST['submit'] !="") {
	...
	if ($db->sql_numrows($db->sql_query_simple("SELECT rid FROM ".$prefix."_rss WHERE headlinesurl='$url'")) > 0) {
		$err_url = "<font color=\"red\">"._ERROR5_1."</font><br>";
		$err = 1;
	}		
	
	if (!$err) {
		$db->sql_query_simple("INSERT INTO ".$prefix."_rss VALUES (NULL, '$site_name', '$url')");
