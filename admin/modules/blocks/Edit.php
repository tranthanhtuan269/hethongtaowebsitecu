...
$result = $db->sql_query_simple("SELECT bkey, title, url, bposition, weight, active, refresh, blanguage, blockfile, view, expire, action, link, module, showtitle FROM ".$prefix."_blocks WHERE bid='$id'");
if(empty($id) || $db->sql_numrows($result) != 1) {
	header("Location: ".$adm_modname.".php");
	die();
}

list($tip, $title, $url, $bposition, $weight, $active, $refresh, $blang, $blockfile, $view, $expire, $action, $link, $bmodule, $showtitle) = $db->sql_fetchrow_simple($result);
...
if(isset($_POST['submit']) && $_POST['submit'] !="") {
	...
	if($bkey==2 && ($url!="" || $headline!=0)) {
		if ($headline != 0) {
			$result = $db->sql_query_simple("select sitename, headlinesurl from ".$prefix."_rss where rid='$headline'");
			$row = $db->sql_fetchrow_simple($result);
			$title = $row['sitename'];
	...
	if ($oldposition != $bposition) {
		$result = $db->sql_query_simple("SELECT weight FROM ".$prefix."_blocks WHERE bposition='$bposition' ORDER BY weight DESC");
		$row = $db->sql_fetchrow_simple($result);
		$weight = $row['weight'];
		$weight++;
	}
	...
	if(!$err) {
		...
		$db->sql_query_simple("UPDATE ".$prefix."_blocks SET bkey='$bkey', title='$title', url='$url', bposition='$bposition', active='$active', refresh='$refresh', blockfile='$blockfile', view='$view', expire='$expire_up', action='$action', link='$link', module='$bmodule_up', showtitle='$showtitle' WHERE bid='$id'"); //, weight='$weight'
		fixweight();
		blist();
...
echo "<table border=\"0\" width=\"100%\" cellspacing=\"0\" cellpadding=\"3\" class=\"tableborder table table-bordered\">\n";
