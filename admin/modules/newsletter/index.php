...
$total = $db->sql_numrows($db->sql_query_simple("SELECT * FROM ".$prefix."_newsletter WHERE status='2'"));
$result = $db->sql_query_simple("SELECT  id, email, html, time, newsletterid FROM ".$prefix."_newsletter WHERE status='2' ORDER BY time DESC LIMIT $offset, $perpage");
if($db->sql_numrows($result) > 0) {
	...
	echo "<div id=\"{$adm_modname}_main\"><form action=\"modules.php?f=$adm_modname&page=$page\" name=\"frm\" method=\"POST\" onsubmit=\"return checkQuick(this);\"><table border=\"0\" width=\"100%\" cellspacing=\"0\" cellpadding=\"3\" class=\"tableborder table table-bordered\">\n";
	echo "<tr>\n";
	...
	echo "<td class=\"row3sd hidden-xs\" align=\"center\" width=\"100\">"._CREATEDATE."</td>\n";
	echo "<td class=\"row1sd hidden-xs\" align=\"center\" width=\"80\">"._NTYPE."</td>\n";
	...
	echo "</tr>\n";
	$i =0;
	while(list($id, $email, $ntype, $time, $newsletterid) = $db->sql_fetchrow_simple($result)) {
		...
		echo "<td align=\"center\" class=\"$css hidden-xs\">$ntype</td>\n";
	...
	echo "<tr><td colspan=\"8\" class=\"row3\"><input type=\"submit\" name=\"submit\" value=\""._DELCHOOSED."\"> </td></tr>";
	echo "</form></table></div><br>";
	//<input type=\"button\" value=\""._ADDEMAIL."\" onclick=\"window.location='?f=$adm_modname&do=add'\">
}else{
	OpenDiv();
