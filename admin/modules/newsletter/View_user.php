...
$result = $db->sql_query_simple("SELECT email, newsletterid FROM ".$prefix."_newsletter WHERE id=$id");
if (empty($id) || $db->sql_numrows($result) != 1) die();

list($email, $newsletterid) = $db->sql_fetchrow_simple($result);
...
if ($newsletterid != "0") {
	...
	$total = $db->sql_numrows($db->sql_query_simple("SELECT * FROM ".$prefix."_newsletter_send WHERE ($sqlseld)"));
	$result = $db->sql_query_simple("SELECT  id, subject, send FROM ".$prefix."_newsletter_send WHERE ($sqlseld) ORDER BY send DESC LIMIT $offset, $perpage");
