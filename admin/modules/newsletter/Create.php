...
if(isset($_POST['subup']) && $_POST['subup'] == 1) {
	...
	if(!$err) {
		$result_text = $db->sql_query_simple("SELECT id, email, html, checkkey FROM ".$prefix."_newsletter WHERE status=2 AND activateCode IS NULL LIMIT $bcc_per_mail");
		$insertQuery = $db->sql_query_simple("INSERT INTO ".$prefix."_newsletter_send (subject, text, html, send) VALUES ('$title', '$ntypetext', '$ntypehtml', '".TIMENOW."')");
		$result_id_send = $db->sql_query_simple("SELECT LAST_INSERT_ID()");
		list($idsend) = $db->sql_fetchrow_simple($result_id_send);
		...
		if (($rowsCount > 0) && ($db->sql_affectedrows($insertQuery) > 0)) {
			...
			$subject = $title;
			if ($smtp_mail == 1) $m = new Mail($adminmail, $adminmail, $subject, $ntypehtml, "SMTP", $smtp_host, $smtp_username, $smtp_password, $smtp_port);
			else $m = new Mail($adminmail, $adminmail, $subject, $ntypehtml);
			$mailIdList = array();
			while (list($id, $email, $nhtml, $checkkey) = $db->sql_fetchrow_simple($result_text)) {
