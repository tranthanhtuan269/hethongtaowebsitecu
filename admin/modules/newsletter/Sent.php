...
$total = $db->sql_numrows($db->sql_query_simple("SELECT*FROM ".$prefix."_newsletter_send"));
$result = $db->sql_query_simple("SELECT  id, subject, send FROM ".$prefix."_newsletter_send ORDER BY send DESC LIMIT $offset, $perpage");
if($db->sql_numrows($result) > 0) {
	...
	echo "<br><div id=\"{$adm_modname}_main\"><form action=\"modules.php?f=$adm_modname&do=$do&page=$page\" name=\"frm\" method=\"POST\" onsubmit=\"return checkQuick(this);\"><table align=\"center\" border=\"0\" width=\"100%\" cellspacing=\"0\" cellpadding=\"3\" class=\"tableborder table table-bordered\">\n";
	echo "<tr>\n";
	...
	echo "</tr>\n";
	$i =0;
	while(list($id, $title, $time) = $db->sql_fetchrow_simple($result)) {
	...
	echo "<input type=\"hidden\" name=\"do\" value=\"quick_do_sent\">";
