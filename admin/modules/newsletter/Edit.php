...
$result = $db->sql_query_simple("SELECT email, html FROM {$prefix}_newsletter WHERE id=$id");
if(empty($id) || $db->sql_numrows($result) != 1) die();

list($email, $ntype) = $db->sql_fetchrow_simple($result);
...
if(isset($_POST['subup']) && $_POST['subup'] == 1) {
	...
	if($stopmail) {
		...
		$db->sql_query_simple("UPDATE ".$prefix."_newsletter SET email='$email', html='$ntype' WHERE id='$id'");
...
echo "<br><form action=\"modules.php?f=$adm_modname&do=$do&id=$id\" method=\"POST\" onsubmit=\"return check(this)\"><table align=\"center\" border=\"0\" width=\"90%\" cellspacing=\"0\" cellpadding=\"3\" class=\"tableborder table table-bordered\">\n";
