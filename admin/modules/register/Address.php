...
$result = $db->sql_query_simple("SELECT address FROM ".$prefix."_contact_add WHERE alanguage='$currentlang'");
list($address) = $db->sql_fetchrow_simple($result);

if(isset($_POST['submit']) && $_POST['submit'] !="") {
	$address = trim(stripslashes(resString($_POST['address'])));
	
	if($db->sql_numrows($result) > 0) {
		$db->sql_query_simple("UPDATE ".$prefix."_contact_add SET address='$address' WHERE alanguage='$currentlang'");
	}else{
		$db->sql_query_simple("INSERT INTO ".$prefix."_contact_add VALUES ('$address', '$currentlang')");
	}	
