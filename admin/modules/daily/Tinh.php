...
if(isset($_POST['subup'])&& $_POST['subup'] == 1) {
	...
	if(empty($title)) {
		$err_title = "<font color=\"red\">Nhập Số điện Thoại</font><br>";
		$err = 1;
	}
	...
	if(!$err) {
		...
		$db->sql_query_simple("INSERT INTO ".$prefix."_tinh (tid, name) VALUES (NULL, '$title')");
...
echo "<div id=\"{$adm_modname}_main\"><form action=\"modules.php?f=$adm_modname&do=$do\" enctype=\"multipart/form-data\" method=\"POST\" onsubmit=\"return check(this);\"><table border=\"0\" width=\"100%\" cellspacing=\"0\" cellpadding=\"4\" class=\"tbl-main\">\n";
echo "<tr><td colspan=\"2\" class=\"header\">Tạo Hotline </td></tr>";
echo "<tr>\n";
...
$total = $db->sql_numrows($db->sql_query_simple("SELECT tid FROM {$prefix}_tinh"));
$result = $db->sql_query_simple("SELECT tid, name FROM {$prefix}_tinh  LIMIT $offset, $perpage");
	...
	echo "<tr><td colspan=\"9\" class=\"header\">Danh Sách Hotline</td></tr>";
	echo "<tr>\n";
	...
	while(list($id, $title) = $db->sql_fetchrow_simple($result)) {
		...
		echo "<td align=\"center\" width=\"30\" class=\"$css\"><a href=\"?f=".$adm_modname."&do=edit_tinh&page=$page&id=$id\" info=\""._EDIT."\"><img border=\"0\" src=\"images/edit.png\"></a></td>\n";
		if($ajax_active != 1) {
			echo "<td align=\"center\" width=\"30\" class=\"$css\"><a href=\"?f=".$adm_modname."&do=delete_tinh&page=$page&id=$id\" title=\""._DELETE."\" onclick=\"ajaxinfoget('modules.php?f=".$adm_modname."&do=delete&page=$page&id=$id\"><img border=\"0\" src=\"images/delete.png\"></td>\n";
		} else {
			echo "<td align=\"center\" width=\"30\" class=\"$css\"><a href=\"?f=".$adm_modname."&do=delete_tinh&page=$page&id=$id\" info=\""._DELETE."\" onclick=\"return confirm('"._DELETEASK1."');\"><img border=\"0\" src=\"images/delete.png\"></td>\n";
		}
		echo "</tr>\n";
		$i++;
	}
