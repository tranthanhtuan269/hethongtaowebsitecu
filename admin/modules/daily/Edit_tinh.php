...
$result = $db->sql_query_simple("SELECT name FROM ".$prefix."_tinh WHERE tid=$id");
...
if (isset($_POST['subup']) && $_POST['subup'] == 1) {
	...
	if (!$err) {
		...
		// 	$images_up = $images;
		// }
		$db->sql_query_simple("UPDATE ".$prefix."_tinh SET name='$title' WHERE tid='$id'");
		updateadmlog($admin_ar[0], $adm_modname, _MODTITLE, _EDIT_NEWS_CAT);
		header("Location: modules.php?f=".$adm_modname."&do=Tinh&page=$page");
	}
}
...
echo "<tr><td colspan=\"2\" class=\"header\">"._MODTITLE." &raquo; Chỉnh sửa Tỉnh Thành</td></tr>";
echo "<tr>\n";
