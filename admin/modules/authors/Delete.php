...
$result = $db->sql_query_simple("SELECT adname, permission FROM ".$prefix."_admin WHERE adacc='$acc'");
if(empty($acc) || $db->sql_numrows($result) != 1) { 
	include("modules/".$adm_modname."/index.php");
	exit;
} else {	
	list($adname, $permission) = $db->sql_fetchrow_simple($result);
	if($adname == "Root" && $permission == 2) {
		die("Hi! Chao ban! Chuc mot ngay tot lanh!");
	}
	$db->sql_query_simple("DELETE FROM ".$prefix."_admin WHERE adacc='$acc'");
