...
?>
    <style>
        @media screen and (max-width: 400px){
            #footermenus_main form:nth-child(1) .table-bordered td{
                width: 100%;
                display: block;
            }
            #footermenus_main form:nth-child(1) .table-bordered td:nth-child(1){
                text-align: left;
            }
...
if(isset($_POST['subup']) && $_POST['subup'] == 1) {
	...
	if(!$err) {
		...
		$db->sql_query_simple("INSERT INTO ".$prefix."_footermenus (mid, parentid, title, url, alanguage, weight, target,images) VALUES (NULL, '$parentid', '$title', '$url', '$currentlang', '$weight', '$newwindow','$images')");
		fixweight_mn();
...
echo "</tr>\n";
$result_cat = $db->sql_query_simple("SELECT mid, title FROM ".$prefix."_footermenus WHERE parentid='0' AND alanguage='$currentlang' ORDER BY weight");
...
$total = $db->sql_numrows($db->sql_query_simple("SELECT * FROM ".$prefix."_footermenus WHERE alanguage='$currentlang'"));
$resultcat = $db->sql_query_simple("SELECT mid, title, active, weight, url, target  FROM ".$prefix."_footermenus WHERE parentid='0' AND alanguage='$currentlang' ORDER BY weight,mid ASC LIMIT $offset, $perpage");
...
function childcat($mid, $text="-") {
	...
	$treeTemp ="";
	$result = $db->sql_query_simple("SELECT mid, title, active, weight, url, target FROM ".$prefix."_footermenus WHERE parentid='$mid' ORDER BY weight,mid ASC");
