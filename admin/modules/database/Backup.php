...
$content = '';
$tableResult = $db->sql_query_simple("SHOW TABLES");
while (list($tableName) = $db->sql_fetchrow_simple($tableResult)) {
	$createTableResult = $db->sql_query_simple("SHOW CREATE TABLE $tableName");
	list($tableName, $query) = $db->sql_fetchrow_simple($createTableResult);
	$query = explode("\n", $query);
	...
	$dbInfoResult = $db->sql_query_simple("SELECT * FROM $tableName");
	$insertQueryPrefix = "INSERT INTO $tableName (";
	$fullInsertQuery = '';
	$start = true;
	while ($dbInfo = $db->sql_fetchrow_simple($dbInfoResult)) {
		if ($start) {
