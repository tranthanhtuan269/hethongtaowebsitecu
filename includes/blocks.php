...
function blist() {
	...
	for($t=0;$t < sizeof($bp_ar);$t++) {
		...
		$fresult = $db->sql_query_simple($fsql);
		while($frow = $db->sql_fetchrow_simple($fresult)) {
    	...
    	if ($bl_ar[5]=="$currentlang") {
    		...
    		if($bl_ar[11]=="all" || $bl_ar[11]=="" || ($home==1 && @in_array("home",$bl_mod_ar)) || ($home!=1 AND @in_array($name,$bl_mod_ar))) {
    			if (intval($bl_ar[8]) != 0 AND intval($bl_ar[8]) <= $now) {
    				if ($bl_ar[9] == "d") {
					$db->sql_query_simple("UPDATE {$prefix}_blocks SET active=0, expire=0 WHERE bid='".intval($bl_ar[12])."'");
					blist();
					header("Location: index.php"); exit();
				} elseif ($bl_ar[9] == "r") {
					$db->sql_query_simple("DELETE FROM ".$prefix."_blocks WHERE bid='".intval($bl_ar[12])."'");
					echo "DELETE FROM ".$prefix."_blocks WHERE bid='".intval($bl_ar[12])."'";
					$db->sql_query_simple("OPTIMIZE TABLE ".$prefix."_blocks");
