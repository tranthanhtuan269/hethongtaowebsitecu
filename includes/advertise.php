...
function advertising($bnid) 
{
	...
	$bnid = intval($bnid);
	$result = $db->sql_query_simple("SELECT bwidth, bheight, abs, type FROM ".$prefix."_advertise_banners WHERE active='1' AND bnid='$bnid'");
	if($db->sql_numrows($result) > 0) {
		list($bwidth, $bheight, $absbn, $bntype) = $db->sql_fetchrow_simple($result);
		if($bntype == 0) {
			$result_bn = $db->sql_query_simple("SELECT id, target, images, imgtext, module FROM ".$prefix."_advertise WHERE bnid='$bnid' AND alanguage='$currentlang' AND active='1' ORDER BY weight");
		} else {
			$result_bn = $db->sql_query_simple("SELECT id, target, images, imgtext, module FROM ".$prefix."_advertise WHERE bnid='$bnid' AND alanguage='$currentlang' AND active='1' ORDER BY rand() DESC");
		}
		if (($bntype == 1) && ($db->sql_numrows($result_bn) > 1)) {
			...
			$imgtextArr = array();
			$bmodulesArr = array();
			while ($tempArr = $db->sql_fetchrow_simple($result_bn)) list($idArr[], $targetArr[], $imagesArr[], $imgtextArr[], $bmodulesArr[]) = $tempArr;
			$advc .= "\n<script>\n";
		...
		if ($db->sql_numrows($result_bn) > 0) {
			$bmod_arr = array();
			while($tempArr = $db->sql_fetchrow_simple($result_bn)) {
				...
				if($bmodules == "all" || (in_array("home",$bmod_arr) && ($home == 1)) || in_array($module_name,$bmod_arr)) {
					...
					if($bnext == "swf") {
						...
						$advc .= "<div class=\"advertise\" align=\"center\"><a href=\"$urlsite/click.php?id=$id\" target=\"$bntarget\" title=\"$imgtext\"><img class='img-responsive' border=\"0\" src=\"$urlsite/$path_upload/adv/$images\" ";
						if($absbn == 1) {
							$advc .= " height=\"$bheight\"";
						}
						$advc .= "></a></div>";
