...
function newsstart() {
	...
	$result = $db->sql_query_simple("SELECT id, title, link, time, images, description, imgtext FROM ".$prefix."_start_list WHERE active='1' AND alanguage='$currentlang' ORDER BY weight DESC LIMIT 6");
	if($db->sql_numrows($result) > 0) {
		...
		$i =0;
		while(list($id, $title, $link, $time, $images, $hometext, $imgtext) = $db->sql_fetchrow_simple($result)) {
			$i ++;
