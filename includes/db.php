...
class sql_db
{
	...
	function sql_query_simple($query = "", $transaction = FALSE) {
		if (!empty($query)) {
			$this->query_result = @mysql_query($query);
		}
		return $this->query_result;
	...
	function sql_fetchrow_simple($query_id = 0) {
		if($query_id) {
	...
	function sql_fetchrow_array($query_id = 0){
		...
		if($query_id) {
			$result = array();
			$i = 0;
			while ($r = mysql_fetch_array($query_id, MYSQL_ASSOC)) {
				$result[$i] = $r;
				$i++;
			}
			$this->time += (get_microtime()-$stime);
