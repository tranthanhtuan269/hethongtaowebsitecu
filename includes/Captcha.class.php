...
class CAPTCHA
{
	...
	function getImage()
	{
		...
		$db->sql_query_simple("DELETE FROM {$prefix}_captcha WHERE UNIX_TIMESTAMP(NOW()) >= UNIX_TIMESTAMP(captchaTime) + {$this->timeout}");
		...
		$db->sql_query_simple("INSERT INTO {$prefix}_captcha VALUES ('$uniqueKey','$text', NOW())");
	...
	function isValid($text)
	{
		...
		$db->sql_query_simple("SELECT text FROM {$prefix}_captcha WHERE uniqueKey='".$escape_mysql_string($_SESSION[CAPTCHA_SESS])."'");
		if ($db->sql_numrows() > 0)
		{
			list($realText) = $db->sql_fetchrow_simple();
			if ($text != $realText) return false;
			$db->sql_query_simple("DELETE FROM {$prefix}_captcha WHERE uniqueKey='".$escape_mysql_string($_SESSION[CAPTCHA_SESS])."'");
			return true;
		}
		else return false;
	}
